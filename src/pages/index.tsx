...
// Add safety checks in useMemo
  const filteredProducts = useMemo(() => {
    ...
    switch(sortOption) {
      case 'price-asc':
        filtered.sort((a, b) => {
          const priceA = (a?.discount || 0) > 0 ? (a?.price || 0) * (1 - (a?.discount || 0) / 100) : (a?.price || 0)
          const priceB = (b?.discount || 0) > 0 ? (b?.price || 0) * (1 - (b?.discount || 0) / 100) : (b?.price || 0)
          return priceA - priceB
        })
        break
      case 'price-desc':
        ...
        break
      case 'newest':
        filtered.sort((a, b) => new Date(b?.added_at || 0).getTime() - new Date(a?.added_at || 0).getTime())
        break
      case 'discount':
...
export async function getStaticProps() {
  ...
  try {
    ...
    const [productsData, vendors, categories] = await Promise.all([
      parseResponse(featuredProductsRes),
      parseResponse(vendorsRes),
      parseResponse(categoriesRes)
    ])
    ...
    if (productsData) {
      ...
      } else if (productsData.hits && Array.isArray(productsData.hits)) {
